...
function updateLayers() {
    ...
    for (layer in layers) {
        ...
        const position = (layers[layer].position !== undefined ? layers[layer].position : layer);
        
        if (!isNaN(row)) TREE_LAYERS[row].push({ layer, position });
        else OTHER_LAYERS[row].push({ layer, position });
